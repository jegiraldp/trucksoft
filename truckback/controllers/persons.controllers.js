...
export const getPersons = async (req, res) => {
  ...
  export const newPerson = async (req, res) => {
    try {
      const { id, firstName,lastName,email,phone,address,isProvider,isCustomer,isEmployed,isNatural,isEmpresa} = req.body;
      const [result] = await pool.query(
        ...
        [id, firstName,lastName,email,phone,address,isProvider,isCustomer,isEmployed,isNatural,isEmpresa]
      );
      res.json(result);
    } catch (error) {
      return res.status(500).json({ mensaje: error.message });
